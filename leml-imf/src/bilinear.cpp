...
public:
	...
	double fun(double *w) {
		...
		if (prob->isSparseFeat)
			smat_x_dmat(X, w, k, XW);
		else
			dmat_x_dmat_rowmajor(1.0, XX, false, w, false, 0.0, XW, (size_t)prob->m, (size_t)k, (size_t)prob->f);
	...
	void grad(double *w, double *g){
		...
		if (prob->isSparseFeat) {
			smat_t &X = *(prob->X), Xt; Xt = X.transpose();
			smat_x_dmat(2.0*Cp, Xt, XWHTH, prob->k, w, g); // g = 2.0*Cp * Xt * XWHTH + w
			//print_dmat(g, prob->f, prob->k);
		}
		else {
			double *XX = prob->XX;
			memcpy(g, w, (size_t)(prob->f*prob->k)*sizeof(double));
			dmat_x_dmat_rowmajor(2.0*Cp, XX, true, XWHTH, false, 1.0, g, (size_t)prob->f, (size_t)prob->k, (size_t)prob->m);
			//do_axpy(1.0, w, g, prob->f*prob->k);
			//print_dmat(g, prob->f, prob->k);
		}
	}
	void Hv(double *s, double *Hs){
		...
		if (prob->isSparseFeat) {
			...
			double *XX = prob->XX;
			dmat_x_dmat_rowmajor(1.0, XX, false, s, false, 0.0, XS, (size_t)prob->m, (size_t)prob->k, (size_t)prob->f);
			doVM(1.0, XS, HTH, 0.0, XSHTH,prob->m, prob->k);
			memcpy(Hs, s, (size_t)(prob->f*prob->k)*sizeof(double));
			dmat_x_dmat_rowmajor(2.0*Cp, XX, true, XSHTH, false, 1.0, Hs, (size_t)prob->f, (size_t)prob->k, (size_t)prob->m);
			//do_axpy(1.0, s, Hs, prob->f*prob->k);
		}
	}
};
...
public:
	...
	void barXv(double *v, double *barXv) {
		...
		if (prob->isSparseFeat)
			...
			dmat_x_dmat_rowmajor(1.0, XX, false, v, false, 0.0, XV, (size_t)prob->m, (size_t)k, (size_t)prob->f);
...
#pragma omp parallel for schedule(dynamic,50) shared(Y,barXv)
	...
	void barXTu(double *u, double a, double *W0, double *barXTu) {
		...
		if (prob->isSparseFeat) {
			...
			double *XX = prob->XX;
			memcpy(barXTu, W0, (size_t)(prob->f*prob->k)*sizeof(double));
			dmat_x_dmat_rowmajor(a, XX, true, UH, false, 1.0, barXTu, (size_t)prob->f, (size_t)k, (size_t)prob->m);
		}
	}
...
std::vector<int> subsample(int K, int N) {
	...
	for(int i = 0; i < K; i++) {
		int j = i + rand() % (N - i);
		int tmp = subset[i]; subset[i] = subset[j]; subset[j] = tmp;
	}
	subset.resize(K);
	return subset;
}
...
void bilinear_train(const bilinear_problem *prob, const bilinear_parameter *param, double *w, double *walltime, double *cputime) {
	double *y = prob->Y->val_t;
	...
	if(prob->W) 
		for(size_t i = 0; i < w_size; ++i) w[i] = prob->W[i];

	for(size_t idx = 0; idx < nnz_Y; ++idx)
		if(y[idx] > 0) pos++; 
	neg = nnz_Y - pos;

	double primal_solver_classification_tol = eps*(double)std::max(std::min(pos,neg), 1UL)/(double)nnz_Y;
		...
		case L2R_BILINEAR_LS:
			{
				...
				TRON tron_obj(fun_obj, param->eps, param->max_tron_iter, param->max_cg_iter);
		...
		case L2R_BILINEAR_LS_FULL:
			{
				...
				//TRON tron_obj(fun_obj, param->eps, param->max_tron_iter, param->max_cg_iter);
				TRON tron_obj(fun_obj, primal_solver_regression_tol, param->max_tron_iter, param->max_cg_iter);
				tron_obj.set_print_string(liblinear_print_string);
				tron_obj.tron(w, false);  // prob->W is the initial
				delete fun_obj;
				break;
...
double auc(const dvec_t& dec_values, const ivec_t& ty, ivec_t& indices, double threshold, size_t *numerator, size_t *denom1, size_t *denom2) {
	double roc  = 0;
...
#pragma omp parallel for 
	for(size_t inst = 0; inst < nr_insts; ++inst){
		...
		sum_auc += auc(predY, trueY, indices, threshold, &numerator, &denom1, &denom2);
		int correct_cnt = 0;
		for(int p = 0; p < top_p; p++) {
			if(trueY[indices[p]]!=-1) 
				correct_cnt++;
			if(nr_true_pos!=0)
				top_acc[p] += (double)correct_cnt/(double)std::min(p+1, nr_true_pos);
			else 
				top_acc[p] += 1.0;
...
/*
void bilinear_predict(const bilinear_problem *prob, const double *W, const double *H, 
		int top_p, double *avg_auc, double *micro_F1, double *avg_top_acc, double *avg_top_real_acc, double *walltime, double *cputime) {
	smat_t &Y = *(prob->Y);
	smat_t &X = *(prob->X);
	int nr_threads = omp_get_max_threads();
	int rank = (int)prob->k;
...
class spherical_cluster{
	public:
		...
		spherical_cluster(size_t n, int dim, int nr_cluster, double* cluster_idx, double *centroids) {
			this->n = n;
			this->dim = dim;
			...
			for(size_t i=0; i<n; i++) {
				...
				if((int)(cluster_idx[i]) >= 50 || (int)(cluster_idx[i] < 0)) {
					printf("i=%d cluster_idx %g (%d)\n", i, cluster_idx[i], (int)(cluster_idx[i]));
				} */
