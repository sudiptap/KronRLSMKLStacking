...
void dgemv_(char *trans, ptrdiff_t *m, ptrdiff_t *n, double *alpha, double *A, ptrdiff_t *lda, double *x, ptrdiff_t *incx, double *beta, double *y, ptrdiff_t *incy);
}
...
// y = alpha*A*x + beta*y: A is stored in column major!
void dmat_x_vec(double alpha, double *A, bool trans_A, double *x, double beta, double *y, size_t m, size_t n) {
	ptrdiff_t mm = (ptrdiff_t)m, nn = (ptrdiff_t)n;
	ptrdiff_t lda = trans_A? nn:mm;
	ptrdiff_t inc = 1;
	char transpose = 'T', notranspose = 'N'; 
	char *transa = trans_A? &transpose: &notranspose;
	dgemv_(transa, &mm, &nn, &alpha, A, &lda, x, &inc, &beta, y, &inc);
}
...
void dmat_x_dmat_rowmajor(double alpha, double *A, bool trans_A, double *B, bool trans_B, double beta, double *C, size_t m, size_t n, size_t k) {
	ptrdiff_t mm = (ptrdiff_t)m, nn = (ptrdiff_t)n, kk = (ptrdiff_t)k;
	ptrdiff_t lda = trans_A? mm:kk, ldb = trans_B? kk:nn, ldc = nn;
	char transpose = 'T', notranspose = 'N'; 
	char *transa = trans_A? &transpose: &notranspose;
	char *transb = trans_B? &transpose: &notranspose;
	dgemm_(transb, transa, &nn, &mm, &kk, &alpha, B, &ldb, A, &lda, &beta, C, &ldc);
}
...
// prints M, where M is row-major
void print_dmat(double *M, size_t m, size_t n) {
