...
class smat_t{
	...
	public:
		...
		// IO methods - added by Donghyuk Shin
		void load_from_array(long _rows, long _cols, long _nnz, unsigned *rowList, unsigned *colList, double *valList);
		...
		void print_mat(int host){
			for(size_t c = 0; c < cols; ++c) if(col_ptr[c+1]>col_ptr[c]){
...
class smat_subset_iterator_t: public entry_iterator_t{
	public:
		enum {ROWMAJOR, COLMAJOR};
		// major: smat_iterator_t::ROWMAJOR or smat_iterator_t::COLMAJOR
		smat_subset_iterator_t(const smat_t& M, int const *subset, int size, int major = ROWMAJOR);
