...
// ADDED BY DONGHYUK SHIN
void smat_t::load_from_array(long _rows, long _cols, long _nnz, unsigned *rowList, unsigned *colList, double *valList){
	...
	for(long idx = 0; idx < _nnz; idx++){
		...
		row_ptr[rowList[idx]+1]++;
		col_ptr[colList[idx]+1]++;
		tmp_row_idx[idx] = rowList[idx];
		tmp_col_idx[idx] = colList[idx];
		tmp_val[idx] = valList[idx];
		perm[idx] = idx;
	}
