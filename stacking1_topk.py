  ...
  for foldid in fold:
    for fd in range(1, foldid+1):
       ...
       for e in kvalues:
         K_set.append(int(e))      
       f.close()
       f = open(path+'kronrls_topK.txt','w')
       for K in K_set:
         topk_ratio = []
         ...
         breg = OneVsRestClassifier(svm.SVC(kernel='rbf',  probability=True,random_state=np.random.RandomState(0), max_iter=1000))
         ...
         for e in pair[1, -K:]:
             if e == 1.0:
                 cnt = cnt + 1 
         topk_ratio = cnt / sums
         #print('top ',K, ' capture rate is ',topk_ratio)
         print(topk_ratio)
         f.write(str(topk_ratio) + '\n')
