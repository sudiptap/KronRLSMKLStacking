function [] = sample_topk(nfolds, nruns, dataname, path)
%clear
seed = 12345678;
rand('seed', seed);
%nfolds = 10; nruns=5;
...
for run=1:nruns
    ...
    for fold=1:nfolds
        ...
        foldpath =strcat(path,'fold',num2str(fold),'/');
        mkdir(foldpath);
        copyfile('predictions_all.txt', foldpath);
        copyfile('predictions_test_all.txt', foldpath);
        copyfile('label_train_all.txt', foldpath);
        copyfile('label_test_all.txt', foldpath);
    end
